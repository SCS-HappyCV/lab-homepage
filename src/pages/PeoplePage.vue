...
<template>
  <main class="people-page">
    ...
    <section class="people-directory" aria-label="成员列表">
      ...
      <div v-if="!isMember" class="auth-hint-bar">
        <Lock :size="15" />
        <span>成员登录后可查看联系方式、成果、经历等完整信息</span>
        <button type="button" class="auth-hint-link" @click="requestLogin">
          点击登录
        </button>
      </div>
            ...
            <div class="member-grid">
              <article v-for="member in group.members" :key="member.id" class="member-card">
                ...
                <div class="member-actions">
                  <a v-if="isMember" :href="`mailto:${member.email}`" aria-label="发送邮件">
                    <Mail :size="17" />
                  </a>
    ...
    <aside v-if="selectedMember" class="profile-drawer" role="dialog" aria-modal="true" aria-label="成员详情">
      ...
      <div v-if="isMember" class="drawer-section">
        <h3>代表成果</h3>
        <ul>
      ...
      <div v-if="isMember && selectedMember.destination" class="drawer-section destination-panel">
        <h3>毕业去向</h3>
